...
export async function GET(
  ...
  try {
    ...
    const url = buildFrcUrl(parsed.season, parsed.eventCode, "rankings");
    ...
    if (res.status === 404)
      return NextResponse.json(
        { error: "Bu etkinlik için henüz sıralama yok (etkinlik aktif değil veya FMS’te kayıt yok).",
        rankings: null,
      });
    if (!res.ok) {
      const text = await res.text();
      console.warn("[frc/rankings] FRC API", res.status, text.slice(0, 150));
      return NextResponse.json({
    ...
    try {
      ...
      return NextResponse.json({
        error: "FRC API geçersiz yanıt; sıralama şu an gösterilemiyor.",
        rankings: null,
      });
    }
    return NextResponse.json(data);
  } catch (e) {
    console.error("[frc/rankings]", e);
    return NextResponse.json({
      error: "Sıralama alınamadı.",
      rankings: null,
    });
  }
}
